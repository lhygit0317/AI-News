...
> **目标读者：零基础人员**
> **预计耗时：1-2 小时完成全部搭建**
> **当前状态：定时任务已暂停。系统仅支持手动生成报告；飞书和邮件需显式手动发送。**
...
10. [第八步：定时任务暂停状态](#10-第八步定时任务暂停状态)
11. [日常维护](#11-日常维护)
12. [故障排查](#12-故障排查)
...
一句话：**一个行业情报助手，目前定时任务已暂停，只在手动触发时搜新闻、写周报。**
...
手动触发  ──→  搜过去7天的新闻
   ...
   5个AI同时搜索（电源/液冷/AI模型/AI方案/太空+人力）
        ↓
   自动写报告 → 默认仅保存本地 → 显式确认后再推送
```

在内容和格式调整完成前，不创建自动定时发送任务。
...
**充值建议：** 首次充 ¥50-100 就够了，每周一次报告大概消耗 ¥1-3。
...
3. 合并结果生成报告
4. 保存本地 Markdown 报告
5. 如需测试推送，先使用 `--dry-run` 预览，再显式执行飞书或邮件脚本
...
定时任务已暂停。当前不要创建 `hermes cron`，也不要配置自动飞书或自动邮件发送。

恢复定时前需要先完成内容和格式确认；恢复时再由负责人明确给出发送时间、接收群和收件人。
...
- API Key 余额充足（每周约消耗 ¥1-2，具体取决于模型和搜索量）
